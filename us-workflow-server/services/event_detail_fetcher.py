...
class EventDetailFetcher:
    ...
    def _apply_result(self, event: Event, result: Dict[str, Any]) -> Dict[str, Any]:
        now = _now_ms()
        ...
        if result.get("success"):
            ...
            logger.info(f"[{event.uuid[:8]}] 事件详情补抓成功")
            return {
                "success": True,
                "status": DETAIL_STATUS_SUCCESS,
        ...
        if status == DETAIL_STATUS_NOT_FOUND:
            logger.warning(f"[{event.uuid[:8]}] 事件详情返回 404，后续将跳过")
        else:
            logger.warning(
                f"[{event.uuid[:8]}] 事件详情补抓失败: status={event.detail_fetch_status}, "
                f"http_status={http_status}, error={event.detail_fetch_error}"
            )
        return {
            "success": False,
            "status": event.detail_fetch_status,
            "http_status": http_status,
            "error": event.detail_fetch_error,
            "uuid": event.uuid,
        }

    def refresh_single_event(self, uuid: str) -> Dict[str, Any]:
        event = self.db.query(Event).filter(Event.uuid == uuid).first()
        if not event:
            raise ValueError(f"事件不存在: {uuid}")
        result = self._fetch_one(event)
        applied = self._apply_result(event, result)
        self.db.commit()
        return applied
    ...
    def fetch_missing_details(self, limit: Optional[int] = None) -> Dict[str, int]:
        ...
        with ThreadPoolExecutor(max_workers=settings.DETAIL_FETCH_CONCURRENCY) as executor:
            ...
            for future in as_completed(future_map):
                ...
                if applied["success"]:
                    stats["success"] += 1
                elif applied["status"] == DETAIL_STATUS_NOT_FOUND:
                    stats["not_found"] += 1
                else:
                    stats["failed"] += 1
